...
class ImgDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        img = self.extractor(images=Image.open(config.data_path + img),return_tensors='pt')

        return {'guid': guid,'img': img,'tag': tag}
...
if __name__ == '__main__':
    data_loader = DataLoader(getImgDataset(config.train_combined_path), batch_size=config.batch_size, shuffle=True)
    pretrained = ViTModel.from_pretrained(config.pretrained_model_img,config=config.pretrained_model_img)
